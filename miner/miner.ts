...
export class Miner {
    ...
    if (!actionPerformed) {
        ...
        for (let y: number = 0; y < this.field[x].length; y++) {
          if (this.field[x][y] === -1 && (probability[x] === undefined || probability[x][y] === undefined)) {
            ...
            if (probability[x][y] === undefined) {
              probability[x][y] = 2 * this.mines / blanksTotal;
              probabilityCount[x][y] = 2;
            }
          }
        }
      }
      ...
      for (let x: number = 0; x < probability.length; x++) {
        ...
        for (let y: number = 0; y < probability[x].length; y++) {
          ...
          const prob = probability[x][y] / (probabilityCount[x][y] + probabilityCount[x][y] % 2);
  ...
  private getAround(sx: number, sy: number, type: number) {
    let count: number = 0;
  ...
  private performAround(sx: number, sy: number, type: number) {
  ...
  private flagMine(x, y) {
    if (this.justClicked.indexOf(x + '-' + y) >= 0) {
      return false;
    }
    this.justClicked.push(x + '-' + y);
    this.mines--;
    browser.actions().click(element(by.id((x + 1) + '_' + (y + 1))), protractor.Button.RIGHT).perform().then(() => {
      if (this.mines === 0){ // finish if need and wait for alert
        for (let x: number = 0; x < this.field.length; x++) {
          for (let y: number = 0; y < this.field[x].length; y++) {
            if (this.field[x][y] === -1) {
              this.openSquare(x, y);
        ...
        browser.wait(waitingForAlert).then((alert) => {
          try {
